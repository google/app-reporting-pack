...
def get_new_date_for_missing_incremental_snapshots(
  bq_client: bigquery.Client, bq_dataset: str, table_name: str
) -> None:
  ...
  try:
    job = bq_client.query(
      'SELECT TABLE_SUFFIX, new_start_date ' f'FROM `{base_table_id}_missing`'
    )
...
def restore_missing_bid_budgets(
  ...
  dates = [
    date.strftime('%Y-%m-%d')
    for date in pd.date_range(days_ago_28, days_ago_1).to_pydatetime().tolist()
  ]
  ...
  change_history = report_fetcher.fetch(
    queries.ChangeHistory(days_ago_28, days_ago_1), customer_ids
  ).to_pandas()

  # Filter rows where budget change occurred
  budgets = change_history.loc[
    (change_history['old_budget_amount'] > 0)
  ...
  campaign_ids = report_fetcher.fetch(
    queries.CampaignsWithSpend(days_ago_28, days_ago_1), customer_ids
  ).to_pandas()
  logging.info(
  ...
  current_bids_budgets_active_campaigns = report_fetcher.fetch(
    queries.BidsBudgetsActiveCampaigns(), customer_ids
  ).to_pandas()
  current_bids_budgets_inactive_campaigns = report_fetcher.fetch(
    queries.BidsBudgetsInactiveCampaigns(days_ago_28, days_ago_1), customer_ids
  ).to_pandas()
  ...
  placeholders = pd.DataFrame(
    data=list(itertools.product(list(campaign_ids.campaign_id.values), dates)),
    columns=['campaign_id', 'day'],
  )

  restored_budgets = restore_history(
    placeholders, budgets, current_bids_budgets, 'budget_amount'
  )
  restored_target_cpas = restore_history(
    placeholders, target_cpas, current_bids_budgets, 'target_cpa'
  )
  restored_target_roas = restore_history(
    placeholders, target_roas, current_bids_budgets, 'target_roas'
  )
...
def restore_missing_cohorts(
  bq_client: bigquery.Client, bq_dataset: str
) -> None:
  try:
    job = bq_client.query(
  ...
  dates = [
    date
    for date in pd.date_range(min(snapshot_dates), max(snapshot_dates))
    .to_pydatetime()
    .tolist()
  ]
...
def restore_history(
  ...
  if not partial_history.empty:
    joined = pd.merge(
      placeholder_df, partial_history, on=['campaign_id', 'day'], how='left'
    )
    ...
    joined[name] = np.where(
      joined['filled_forward'].isnull(), joined[name], joined['filled_forward']
    )
  else:
    joined = pd.merge(
      placeholder_df, current_values, on='campaign_id', how='left'
    )
  if name != 'target_roas':
...
def format_partial_change_history(
  df: pd.DataFrame, dimension_name: str
) -> pd.DataFrame:
...
def main():
  ...
  if args[0].bid_budgets:
    restore_missing_bid_budgets(
      google_ads_client, config, bq_client, bq_dataset
    )
  if args[0].cohorts:

...
AS (
  WITH
    ...
    PreparedData AS (
      SELECT
        PARSE_DATE('%Y-%m-%d', CAST(SP.date AS STRING)) AS day,
        M.account_id,
        M.account_name,
        M.ocid,
        M.currency,
        ...
        SUM(IF(SP.skan_conversion_value IS NULL, SP.skan_postbacks, 0))
          AS sum_null_values,
        SUM(
          IF(CAST(SP.skan_conversion_value AS INT64) = 0, SP.skan_postbacks, 0))
          AS sum_zero_values,
        SUM(
          IF(
            SP.skan_conversion_value IS NOT NULL
              OR CAST(SP.skan_conversion_value AS INT64) != 0,
            SP.skan_postbacks,
            0)) AS sum_wo_null_zero_values,
        SUM(SP.skan_postbacks) AS skan_postbacks,
        SUM(
          IF(
            ACS.bidding_strategy = 'OPTIMIZE_INSTALLS_TARGET_INSTALL_COST',
            SP.skan_postbacks,
            0)) AS tcpi_skan_postbacks,
        SUM(
          IF(
            ACS.bidding_strategy
              = 'OPTIMIZE_IN_APP_CONVERSIONS_TARGET_INSTALL_COST',
            SP.skan_postbacks,
            0)) AS tcpi_advanced_skan_postbacks,
        SUM(
          IF(
            ACS.bidding_strategy IN (
              'OPTIMIZE_IN_APP_CONVERSIONS_TARGET_CONVERSION_COST',
              'OPTIMIZE_INSTALLS_WITHOUT_TARGET_INSTALL_COST'),
            SP.skan_postbacks,
            0)) AS tcpa_skan_postbacks,
        SUM(
          IF(
            ACS.bidding_strategy = 'OPTIMIZE_RETURN_ON_ADVERTISING_SPEND',
            SP.skan_postbacks,
            0)) AS troas_skan_postbacks,
      ...
      LEFT JOIN `{bq_dataset}.AppCampaignSettingsView` AS ACS
        ON SP.campaign_id = ACS.campaign_id
      LEFT JOIN SkanInputSchema AS S
        ON
          CAST(SP.skan_conversion_value AS INT64) = S.skan_conversion_value
          AND ACS.app_id = S.app_id
      LEFT JOIN MappingTable AS M
        ON SP.campaign_id = M.campaign_id
      GROUP BY ALL
    )
  SELECT
    day,
    ...
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_INSTALLS_TARGET_INSTALL_COST',
        sum_null_values,
        0)) AS tcpi_sum_null_values,
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_IN_APP_CONVERSIONS_TARGET_INSTALL_COST',
        sum_null_values,
        0)) AS tcpi_advanced_sum_null_values,
    SUM(
      IF(
        bidding_strategy IN (
          'OPTIMIZE_IN_APP_CONVERSIONS_TARGET_CONVERSION_COST',
          'OPTIMIZE_INSTALLS_WITHOUT_TARGET_INSTALL_COST'),
        sum_null_values,
        0)) AS tcpa_sum_null_values,
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_RETURN_ON_ADVERTISING_SPEND',
        sum_null_values,
        0)) AS troas_sum_null_values,
    SUM(sum_zero_values) AS sum_zero_values,
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_INSTALLS_TARGET_INSTALL_COST',
        sum_zero_values,
        0)) AS tcpi_sum_zero_values,
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_IN_APP_CONVERSIONS_TARGET_INSTALL_COST',
        sum_zero_values,
        0)) AS tcpi_advanced_sum_zero_values,
    SUM(
      IF(
        bidding_strategy IN (
          'OPTIMIZE_IN_APP_CONVERSIONS_TARGET_CONVERSION_COST',
          'OPTIMIZE_INSTALLS_WITHOUT_TARGET_INSTALL_COST'),
        sum_zero_values,
        0)) AS tcpa_sum_zero_values,
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_RETURN_ON_ADVERTISING_SPEND',
        sum_zero_values,
        0)) AS troas_sum_zero_values,
    SUM(sum_wo_null_zero_values) AS skan_values_wo_null_zero,
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_INSTALLS_TARGET_INSTALL_COST',
        sum_wo_null_zero_values,
        0)) AS tcpi_skan_values_wo_null_zero,
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_IN_APP_CONVERSIONS_TARGET_INSTALL_COST',
        sum_wo_null_zero_values,
        0)) AS tcpi_advanced_skan_values_wo_null_zero,
    SUM(
      IF(
        bidding_strategy IN (
          'OPTIMIZE_IN_APP_CONVERSIONS_TARGET_CONVERSION_COST',
          'OPTIMIZE_INSTALLS_WITHOUT_TARGET_INSTALL_COST'),
        sum_wo_null_zero_values,
        0)) AS tcpa_skan_values_wo_null_zero,
    SUM(
      IF(
        bidding_strategy = 'OPTIMIZE_RETURN_ON_ADVERTISING_SPEND',
        sum_wo_null_zero_values,
        0)) AS troas_skan_values_wo_null_zero
  FROM PreparedData
  GROUP BY ALL
);

...
CREATE OR REPLACE FUNCTION `{bq_dataset}.GetNumberOfElements`(
  first_element STRING, second_element STRING, third_element STRING)
RETURNS INT64
AS (
  ARRAY_LENGTH(
    SPLIT(
...
CREATE OR REPLACE FUNCTION `{bq_dataset}.BinText`(
  type STRING, text_length INT64)
RETURNS STRING
AS (
  CASE

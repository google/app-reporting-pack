...
CREATE OR REPLACE VIEW `{bq_dataset}.AssetCohorts`
AS (
  WITH
    ...
    Final AS (
      SELECT
        ...
        LAST_VALUE(installs IGNORE NULLS)
          OVER (
            PARTITION BY
        ...
        LAST_VALUE(inapps IGNORE NULLS)
          OVER (
            PARTITION BY
              day_of_interaction, ad_group_id, asset_id, network, field_type
            ORDER BY lag
        ...
        LAST_VALUE(view_through_conversions IGNORE NULLS)
          OVER (
            PARTITION BY
              day_of_interaction, ad_group_id, asset_id, network, field_type
            ORDER BY lag
        ...
        LAST_VALUE(conversions_value IGNORE NULLS)
          OVER (
            PARTITION BY
              day_of_interaction, ad_group_id, asset_id, network, field_type
            ORDER BY lag
      ...
      FROM
        DistinctAdGroupAssetDimensions,
        UNNEST(GENERATE_ARRAY(1, 90)) AS lag
      LEFT JOIN RawAssetConversionLags
        USING (
          day_of_interaction, ad_group_id, asset_id, network, lag, field_type)
  ...
  SELECT
    ...
    STRUCT(
      ...
      ARRAY_AGG(view_through_conversions ORDER BY lag)
        AS view_through_conversions) AS lag_data
  FROM Final
  WHERE

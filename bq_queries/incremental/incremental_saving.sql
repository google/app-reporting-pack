...
CREATE OR REPLACE TABLE `{bq_dataset}.incremental_asset_performance_{date_iso}` AS
WITH Temp AS (
    SELECT
        *,
...
-- Define view combining saved historical asset performance data 
-- with the latest fetched data
CREATE OR REPLACE VIEW `{target_dataset}.full_asset_performance_view` AS
WITH Temp AS (
    SELECT
        *,
        MIN(day) OVER() AS min_day
...
SELECT * FROM `{bq_dataset}.incremental_asset_performance_*`
UNION ALL
SELECT * EXCEPT (min_day) FROM temp WHERE day > min_day;
